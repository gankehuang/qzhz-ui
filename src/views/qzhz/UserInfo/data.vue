  ...
  <div class="app-container">
    ...
    <el-form ref="form" :model="form" :inline="true" label-width="120px">
      ...
      <el-form-item label="保障人身份证号" prop="guarantorIdCard">
        ...
        <!-- <el-input v-if="isEdit" v-model="form.guarantorIdCard" placeholder="请输入保障人身份证号" /> -->
      </el-form-item><br/>
    ...
    <!-- 添加或修改会员费用对话框 -->
    <el-dialog title="编辑保障人" :visible.sync="open" width="800px" append-to-body>
      <p style="color: blue; text-align: right; cursor: pointer;" @click="editRecord">编辑记录</p>
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        <el-form-item label="保障人姓名" prop="guarantorName">
          <el-input v-model="form.guarantorName" placeholder="请输入保障人姓名" />
        </el-form-item>
        <el-form-item label="身份证号" prop="guarantorIdCard">
          <el-input v-model="form.guarantorIdCard" placeholder="请输入身份证号" />
        </el-form-item>
        <el-form-item label="备注原因" prop="remark">
          <el-input type="textarea" v-model="form.remark"></el-input> 
        </el-form-item>
      </el-form>
      <p style="color: red; margin-left: 120px">此操作存在法律风险，请谨慎操作！</p>
    ...
    <el-dialog title="编辑记录" :visible.sync="openEditRecord" width="1000px" append-to-body>
      <div class="block">
        <el-timeline>
          <el-timeline-item v-for="(record, index) in EditRecordList" :key="index" :timestamp="record.newEditRecord.createTime" placement="top">
            <el-card>
              <p>操作人：{{record.newEditRecord.createBy}}</p>
              <h4 style="color: red;">编辑后：{{record.newEditRecord.guarantorName}}&nbsp;&nbsp;{{record.newEditRecord.guarantorIdCard}}</h4>
              <p v-if="record.oldEditRecord">编辑前：{{record.oldEditRecord.guarantorName}}&nbsp;&nbsp;{{record.oldEditRecord.guarantorIdCard}}</p>
    ...
    <el-dialog title="账单详情" :visible.sync="openlLookRecord" width="1000px" append-to-body>
      <el-table v-loading="loading" :data="BillDetailsList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="账单金额" align="center" prop="billAmount" >
          <template slot-scope="scope">
            <span v-if="scope.row.billType==1||scope.row.billType==2||scope.row.billType==3||scope.row.billType==6">+{{memberCostFormat2(scope.row)}}</span>
            <span v-else-if="scope.row.billType==5">-{{memberCostFormat2(scope.row)}}</span>
...
<script>
import { listGuarantor, getGuarantor, getGuarantor1, delGuarantor, addGuarantor, updateGuarantor, exportGuarantor } from "@/api/qzhz/Guarantor";
import { listEditRecord, getEditRecord, delEditRecord, addEditRecord, updateEditRecord, exportEditRecord } from "@/api/qzhz/EditRecord";
...
export default {
  ...
  created() {
    ...
    getGuarantor1(userId).then(response => {
  ...
  methods: {
    ...
    isSettlementFormat(row, column) {
      return this.selectDictLabel(this.isSettlementTrueOptions, row.isSettlement);
    },
    ...
    addBlackList() {
      ...
      isBlack({blackIdCard: this.form.guarantorIdCard}).then(res => {
        if (res.data.is_black) {
          ...
          this.$confirm(`姓名：${this.form.guarantorName}，身份证号：${this.form.guarantorIdCard}，该保障人将被加入${this.form.joinPlanName}的黑名单，加入成功后将自动退出该计划，确定加入黑名单吗？`, "加入黑名单", {
              confirmButtonText: "确定",
              cancelButtonText: "取消",
              type: "warning"
            }).then(function() {
